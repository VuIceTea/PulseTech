...
export interface FullCoupon { id: string; code: string; description: string; discountPercent: number; discountAmount: number; minOrderValue: number; maxDiscountValue: number; validFrom: string; validUntil: string; currentUsage: number; maxUsage: number; isActive: boolean; count?: number; usedCount?: number; }
