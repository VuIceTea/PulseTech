        ...
        {registeredEmail ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-4">
              <Field label="Họ và tên" icon={<User size={16} />} type="text" autoComplete="name" value={name} maxLength={AUTH_LIMITS.nameMax} error={errors.name} onChange={(value) => { setName(value); setErrors((current) => ({ ...current, name: undefined, form: undefined })); }} placeholder="Nguyễn Văn A" />
              <Field label="Địa chỉ email" icon={<Mail size={16} />} type="email" autoComplete="email" value={email} maxLength={AUTH_LIMITS.emailMax} error={errors.email} onChange={(value) => { setEmail(value); setErrors((current) => ({ ...current, email: undefined, form: undefined })); }} placeholder="example@gmail.com" />
              <Field label="Mật khẩu" icon={<Lock size={16} />} type="password" autoComplete="new-password" value={password} maxLength={AUTH_LIMITS.passwordMax} error={errors.password} onChange={(value) => { setPassword(value); setErrors((current) => ({ ...current, password: undefined, confirmPassword: undefined, form: undefined })); }} placeholder="8–72 ký tự, gồm hoa, thường, số, ký tự đặc biệt" />
              <Field label="Xác nhận mật khẩu" icon={<Lock size={16} />} type="password" autoComplete="new-password" value={confirmPassword} maxLength={AUTH_LIMITS.passwordMax} error={errors.confirmPassword} onChange={(value) => { setConfirmPassword(value); setErrors((current) => ({ ...current, confirmPassword: undefined, form: undefined })); }} placeholder="Nhập lại mật khẩu" />
              <button type="submit" disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-3 text-xs font-bold uppercase tracking-wider text-white hover:bg-primary-dark disabled:cursor-not-allowed disabled:opacity-60 transition-colors"><UserPlus size={17} />{loading ? 'Đang gửi email...' : 'Tạo tài khoản'}</button>
            </form>
...
function Field({ label, icon, type: initialType, autoComplete, value, maxLength, error, onChange, placeholder }: { label: string; icon: React.ReactNode; type: string; autoComplete: string; value: string; maxLength: number; error?: string; onChange: (value: string) => void; placeholder: string }) {
  const [show, setShow] = useState(false);
  const isPassword = initialType === 'password';
  const type = isPassword && show ? 'text' : initialType;
  return <label className="block text-xs font-bold text-gray-700">{label}<span className="relative mt-1 block"><input required type={type} autoComplete={autoComplete} value={value} maxLength={maxLength} onChange={(event) => onChange(event.target.value)} aria-invalid={!!error} placeholder={placeholder} className={`w-full rounded-xl border bg-gray-50 py-2.5 pl-10 pr-10 text-xs text-brand-black outline-none transition-colors ${error ? 'border-primary' : 'border-gray-200 focus:border-primary focus:bg-white'}`} /><span className="absolute left-3.5 top-2.5 text-gray-400">{icon}</span>{isPassword && <button type="button" tabIndex={-1} onClick={() => setShow(!show)} className="absolute right-3.5 top-2.5 text-gray-400 hover:text-brand-black">{show ? <EyeOff size={16} /> : <Eye size={16} />}</button>}</span>{error && <span className="mt-1.5 block text-[11px] text-primary">{error}</span>}</label>;
}